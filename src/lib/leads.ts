...
const ATTRIBUTION_KEY = 'hs_attribution';
...
/**
 * Submits a lead to the backend (database + email + file backup on the server) enriched
...
export async function submitLead(payload: LeadPayload): Promise<void> {
  ...
  try {
    const existing = JSON.parse(localStorage.getItem('heliumsol_leads') || '[]');
