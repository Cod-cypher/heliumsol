// Central place for contact + booking details used across the site.
// Every "Contact us" button points at the /contact enquiry form.
export const BOOKING_URL = "/contact";

export const CONTACT = {
  email: "info@heliumsol.com",
  // Displayed as written; the tel: links in the footer strip everything but
  // digits and the leading +, so this renders as +12028107042 when dialled.
  phone: "+1 (202) 810-7042",
  location: "Remote · Serving clients worldwide",
  responseTime: "We reply within 1 business day",
};
