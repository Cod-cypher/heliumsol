...
export interface ChatMessageDTO {
  ...
  kind: "message" | "notice";
  /** Label on the bubble — "the team", "HeliumSol". Absent for the visitor's own. */
  authorLabel?: string;
  createdAt: string;
}
...
export interface ChatPollResponse {
  ...
  pending?: boolean;
  /**
   * Set on a send response when this turn asked for a person: whether the team
   * was actually notified. The widget turns it into a visible status.
   */
  handoff?: "notified" | "unavailable" | "cooldown";
}
...
export interface ChatAgentContext {
  conversationId: string;
  agentLabel: string;
  /** Already joined, e.g. because the team reopened the link on a second device. */
  joined: boolean;
