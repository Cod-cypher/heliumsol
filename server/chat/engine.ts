...
export { asksForHuman } from "../../shared/chatIntent";
 ...
 * user turns would have it answering the team instead of the visitor.
 */
...
export async function runTurn(
  ...
  // to fetch the team when the team is already on the way.
  ...
  if (conversation.turnCount >= maxTurns()) {
    const result = await ctx.requestHuman("out_of_scope", "Conversation reached the turn limit.", "today");
    return {
      text: result.outcome === "unavailable" ? CHAT_HANDOFF_UNAVAILABLE : CHAT_TURN_CAP,
 ...
 *   offering to fetch the team when the team is already on the way reads as an assistant
