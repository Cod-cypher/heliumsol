 ...
 *
 * SITE_ORIGIN is a hardcoded https://heliumsol.com, which is correct in
...
export function joinBlockHtml(url: string, summary: string | null): string {
  ...
  return `${summaryHtml}<p style="margin:0 0 12px"><a href="${escaped}" style="display:inline-block;padding:12px 20px;background:#142540;color:#ffffff;font:700 14px/1 -apple-system,Segoe UI,sans-serif;text-decoration:none;border:1px solid #142540;border-radius:8px">Join the conversation</a></p><p style="margin:0;font:12px/1.5 -apple-system,Segoe UI,sans-serif;color:#64748b">The link expires in 48 hours. Opening it does not announce you — you join with a button once you are looking at the transcript.</p>`;
}
