...
export const AGENT_COOKIE = "hs_chat_agent";

/** Agent cookie life. Shorter than the link, because a desk is not an inbox. */
const AGENT_COOKIE_MS = 12 * 60 * 60 * 1000;

/**
 * How long a visitor must wait before asking for a person again in the same
 * conversation. Also the debounce on the handoff email. Configurable with
 * CHAT_HANDOFF_COOLDOWN_MINUTES (default 10).
 */
function handoffCooldownMs(): number {
  const raw = Number(process.env.CHAT_HANDOFF_COOLDOWN_MINUTES);
  return (Number.isFinite(raw) && raw > 0 ? raw : 10) * 60 * 1000;
}

/** What asking for a person came to. */
export interface HandoffResult {
  outcome: "notified" | "unavailable" | "cooldown";
  /** Set with "cooldown": minutes until they can ask again. */
 ...
 * in the same sitting and the team has already been emailed about it, above it this
...
function agentLabel(): string {
  return process.env.CHAT_AGENT_LABEL || "HeliumSol";
}
...
/**
 * The nonce for a join link about to be emailed.
 *
 * Always a fresh one, so sending a new link invalidates every link sent before
 * it: only the newest email in the inbox can open the conversation. The one
 * exception is a team member who is in the chat right now — rotating would
 * sign them out mid-conversation — so while it is LIVE the current link stands.
 */
function nextAgentTokenId(conversation: ChatConversation): string {
  const current = liveTokenId(conversation);
  if (current && conversation.status === "LIVE" && conversation.agentJoinedAt) return current;
...
export function chatRoutes(prisma: PrismaClient, deps: ChatDeps): express.Router {
   ...
   * behind it.
   *
   * It carries the same join link as a real handoff would, so the team can drop into
  ...
  async function notifyChatStarted(conversationId: string, firstMessage: string): Promise<void> {
    ...
    // The team already has a handoff email for this chat, whose link must stay
    // the valid one. An opening notice now would only replace it.
    if (conversation.handoffEmailSentAt) return;
    if (!deps.smtpConfigured) return;

    const jti = nextAgentTokenId(conversation);
   ...
   * thread the team was emailed about days ago and has no reason to be looking at.
   *
  ...
  async function notifyChatResumed(conversationId: string, text: string, quietFor: number): Promise<void> {
    ...
    /*
      ...
      would land in the team's inbox describing one event. Requiring the opening
      notification to be older than the gap means a conversation cannot be
      started and resumed in the same breath.
    */
    // The first email the team got about this chat: the opening notice, or a
    // handoff when the very first message asked for a person.
    const firstEmailAt = conversation.startedEmailSentAt || conversation.handoffEmailSentAt;
    if (!firstEmailAt) return;
    if (Date.now() - firstEmailAt.getTime() < RESUMED_GAP_MS) return;
    ...
    // A new link, which invalidates the one in any earlier email.
    const jti = nextAgentTokenId(conversation);
   ...
   *
   * Narrow on purpose. It fires only when nobody had joined — if the team was in the
  ...
  async function notifyVisitorLeft(conversationId: string): Promise<void> {
    ...
    if (!conversation.startedEmailSentAt && !conversation.handoffEmailSentAt) return;
  ...
  /*
    ...
    tell the team somebody walked out every time they followed a link that reloaded
  ...
  */
  ...
  /**
   * Triggers a handoff: mints a fresh join link (invalidating earlier ones),
   * stores its nonce, emails it.
   *
   * Once per conversation per cooldown, which the visitor is told about. The
   * per-visitor ceiling is checked by requestPerson, so starting a new chat
   * does not reset it. `respectCooldown: false` is only for the system
   * re-alerting after a team member dropped out of a live chat.
   */
  async function triggerHandoff(
    ...
    const conversation = await getConversation(prisma, conversationId);
    if (!conversation) return { outcome: "unavailable" };
    if (conversation.status === "LIVE") return { outcome: "notified" }; // already has a person

    const sinceLast = conversation.handoffEmailSentAt
      ? Date.now() - conversation.handoffEmailSentAt.getTime()
      : Infinity;
    if (opts.respectCooldown !== false && sinceLast < handoffCooldownMs()) {
      return {
        outcome: "cooldown",
        retryAfterMin: Math.max(1, Math.ceil((handoffCooldownMs() - sinceLast) / 60000)),
      };
    }

    // A fresh link: the one in any earlier email stops working.
    const jti = nextAgentTokenId(conversation);
    ...
    const updated = await prisma.chatConversation
      .update({
        ...
        data: {
          ...
          // Claimed before sending, so two requests in the same instant cannot
          // both pass the cooldown. Put back below if the send fails.
      ...
      .catch(() => null);
    if (!updated) return { outcome: "unavailable" };

    const release = () =>
      prisma.chatConversation
        .update({
          where: { id: conversationId },
          data: { handoffEmailSentAt: conversation.handoffEmailSentAt },
        })
        .catch(() => {});
    ...
    const facts = handoffFacts(updated, null);
    const sent = await deps.sendMail({
  ...
  /**
   * A visitor asking for a person: the per-visitor ceiling first (so a fresh
   * conversation cannot dodge the cooldown), then the handoff itself.
   */
  async function requestPerson(
    ...
    const gate = checkChatRate("handoff", ip);
    if (!gate.allowed) {
      return {
        outcome: "cooldown",
        retryAfterMin: Math.max(1, Math.ceil((gate.retryAfterSec ?? 60) / 60)),
      };
    }
    return triggerHandoff(conversationId, reason, summary);
  }
  ...
  async function captureLead(conversation: ChatConversation): Promise<void> {
    ...
    const { id } = await deps.persistLead({
      ...
      // not given one yet. Same approach the contact form takes: say
      ...
      goal: "",
      service: "",
      // No budget field in a chat, so this carries where they are based instead.
  ...
  router.post("/:id/message", async (req, res) => {
    ...
    try {
      ...
      //
      // Skipped when this message is itself a request for a person: the handoff
      // email below covers it, and sending both at once would race to own the
      // one valid join link.
      ...
      */
      if (conversation.status === "LIVE" && !agentPresent(id)) {
        ...
        // through: triggerHandoff suppresses a second email inside the cooldown
      ...
      // What happened to a request for a person this turn, so the widget can
      // show the visitor the steps rather than a bare reply.
      let handoff: ChatPollResponse["handoff"];
      if (asksForHuman(text)) {
        const result = await requestPerson(id, ip, "visitor_asked", text.slice(0, 500));
          ...
          content:
            result.outcome === "notified"
              ...
              : result.outcome === "cooldown"
                ? chatHandoffCooldown(result.retryAfterMin ?? 1)
                : CHAT_HANDOFF_UNAVAILABLE,
          authorLabel: CHAT_SIDE_LABEL,
        });
        forcedHandoff = true;
      ...
      const ctx: ToolContext = {
        ...
        requestHuman: async (reason, summary, _urgency) => {
          const result = await requestPerson(id, ip, reason, summary);
          handoff = result.outcome;
          return result;
        },
      };

      // A clear request for a person has already been answered above. Running
      // the model as well produced a second, reworded acknowledgement straight
      // after the first, so the turn is skipped.
      if (!forcedHandoff) {
...
export function chatAgentRoutes(prisma: PrismaClient, deps: ChatDeps): express.Router {
   ...
   * does on open. Mail clients prefetch and scan links — announcing "the team
  ...
  router.post("/:id/join", guard, async (req, res) => {
    ...
    if (!conversation.agentJoinedAt) {
      await prisma.chatConversation
        ...
        .catch(() => {});
      // Deliberately silent. Announcing "the team joined" tells the visitor the
...
function renderHandoffHtml(
  ...
  const rows = Object.entries(facts)
    ...
    .map(
      ([k, v]) =>
        `<tr><td style="padding:6px 12px 6px 0;color:#64748b;font:13px/1.4 -apple-system,Segoe UI,sans-serif;vertical-align:top">${k}</td><td style="padding:6px 0;font:13px/1.4 -apple-system,Segoe UI,sans-serif;color:#0f172a">${escapeHtml(v)}</td></tr>`,
    )
    .join("");
  ...
  return [
    ...
    `<h3 style="font:600 13px/1.4 -apple-system,Segoe UI,sans-serif;color:#64748b;margin:28px 0 8px;text-transform:uppercase;letter-spacing:.04em">Recent messages</h3>`,
    `<pre style="font:12px/1.6 ui-monospace,SFMono-Regular,Menlo,monospace;color:#0f172a;white-space:pre-wrap;margin:0;padding:12px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px">${escapeHtml(transcript)}</pre>`,
  ].join("");
}
