...
/**
 * Asking for a real person, per visitor IP. On top of the per-conversation
 * cooldown, so starting a new chat does not reset it.
 */
const HANDOFF_WINDOWS: Window[] = [
  { limit: 3, windowMs: 60 * 60 * 1000, label: "hour" },
  { limit: 6, windowMs: 24 * 60 * 60 * 1000, label: "day" },
];
...
export type ChatLimit = "start" | "message" | "poll" | "agent" | "handoff";
...
function refusal(kind: ChatLimit, w: Window): string {
  if (kind === "start") {
    return "That is a lot of new conversations from one place. Give it a few minutes, or email info@heliumsol.com and we will pick it up there.";
  }
  if (kind === "agent") {
  ...
  return `You have sent ${w.limit} messages this ${w.label}. Take the human route instead — email info@heliumsol.com and we will reply properly.`;
}
